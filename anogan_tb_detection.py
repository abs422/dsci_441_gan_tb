...
Original file is located at
    https://colab.research.google.com/drive/1QbyftznvMCeaghaFGD6lB-BFZ2C1jJ8m
"""
...
healthy_montgo_path = '/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/Data/Montgo/Healthy'
tb_montgo_path = '/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/Data/Montgo/Tb'

healthy_ind_path = '/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/Data/TBX11K/imgs/extra/da+db/train'
tb_ind_path = '/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/Data/TBX11K/imgs/extra/da+db/val'
...
# Load Tb montgo image data from directory
tb_montgo = []
for file in os.listdir(tb_montgo_path):
    img = Image.open(os.path.join(tb_montgo_path, file)).convert('L')
    img = img.resize((256, 256))
    tb_montgo.append(np.array(img).reshape(256, 256, 1))

X_tb_montgo = np.array(tb_montgo)

# Load healthy montgo image data from directory
healthy_montgo = []
for file in os.listdir(healthy_montgo_path):
    img = Image.open(os.path.join(healthy_montgo_path, file)).convert('L')
    img = img.resize((256, 256))
...
tb_ind = []
for file in os.listdir(tb_ind_path):
    img = Image.open(os.path.join(tb_ind_path, file)).convert('L')
    img = img.resize((256, 256))
    tb_ind.append(np.array(img).reshape(256, 256, 1))

X_tb_ind = np.array(tb_ind)

# Load healthy ind image data from directory
healthy_ind = []
for file in os.listdir(healthy_ind_path):
    img = Image.open(os.path.join(healthy_ind_path, file)).convert('L')
    img = img.resize((256, 256))
    healthy_ind.append(np.array(img).reshape(256, 256, 1))

X_healthy_ind = np.array(healthy_ind)
...
ascore_tb_ind = []
for i in range(0, X_tb_ind.shape[0]):
  train_img = X_tb_ind[i]
  score, qurey, pred, diff = anomaly_detection(train_img)
  ascore_tb_ind = ascore_tb_ind + [score]
  ascore_tb_inddf = pd.DataFrame(ascore_tb_ind)
  ascore_tb_inddf.to_csv('ascore_tb_inddf.csv')


ascore_healthy_ind = []
for i in range(0, X_healthy_ind.shape[0]):
  train_img = X_healthy_ind[i]
  score, qurey, pred, diff = anomaly_detection(train_img)
  ascore_healthy_ind = ascore_healthy_ind + [score]
  ascore_healthy_inddf = pd.DataFrame(ascore_healthy_ind)
  ascore_healthy_inddf.to_csv('ascore_healthy_inddf.csv')
...
sick_tbx = pd.read_csv('/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/ascore_sickdf.csv')
healthy_tbx = pd.read_csv('/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/ascore_traindf.csv')
# healthy_tb = pd.read_csv('/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/model_eval.csv')
tb_tbx = pd.read_csv('/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/ascore_tbdf.csv')
tb_shen = pd.read_csv('/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/ascore_testdf.csv')
tb_montgo = pd.read_csv('/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/ascore_tb_montgodf.csv')
healthy_montgo = pd.read_csv('/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/ascore_healthy_montgodf.csv')

tb_ind = pd.read_csv('/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/ascore_tb_inddf.csv')
healthy_ind = pd.read_csv('/content/drive/MyDrive/DSCI 441 - Anomaly detection in images/ascore_healthy_inddf.csv')

threhold = 9300000

import numpy as np

sick_tbx['label'] = np.where(sick_tbx['0'] > threhold, 1, 0)
sick_tbx['gt'] = 1

healthy_tbx['label'] = np.where(healthy_tbx['0'] > threhold, 1, 0)
healthy_tbx['gt'] = 0

tb_shen['label'] = np.where(tb_shen['0'] > threhold, 1, 0)
tb_shen['gt'] = 1

tb_tbx['label'] = np.where(tb_tbx['0'] > threhold, 1, 0)
tb_tbx['gt'] = 1

tb_montgo['label'] = np.where(tb_montgo['0'] > threhold, 1, 0)
tb_montgo['gt'] = 1

healthy_montgo['label'] = np.where(healthy_montgo['0'] > threhold, 1, 0)
healthy_montgo['gt'] = 0

tb_ind['label'] = np.where(tb_ind['0'] > threhold, 1, 0)
tb_ind['gt'] = 1

healthy_ind['label'] = np.where(healthy_ind['0'] > threhold, 1, 0)
healthy_ind['gt'] = 0

healthy_shen = healthy_tbx[:326]
...
ax = sns.kdeplot(data=healthy_shen['0'], label='Healthy- Shen', ax=ax, shade = True, color= 'darkred')
ax = sns.kdeplot(data=tb_shen['0'], label='Tb-Shenzhen' , ax=ax, shade = True, color = 'darkblue')
...
tb_shen.to_csv('tb_shen.csv')

fig, ax = plt.subplots()

ax = sns.kdeplot(data=healthy_tbx[:3377]['0'], label='Healthy- TbX11K', ax=ax, shade = True, color= 'red')
ax = sns.kdeplot(data=tb_tbx[:500]['0'], label='Tb- TbX11K', ax=ax, shade = True, color= 'lightblue')
...
ax = sns.kdeplot(data=healthy_montgo['0'], label='Healthy- Montgomery', ax=ax, shade = True, color= 'indianred')

ax = sns.kdeplot(data=tb_montgo['0'], label='Tb-Montgomery' , ax=ax, shade = True, color = 'blue')
# ax = sns.kdeplot(data=healthy_ind['0'], label='Healthy- Indian', ax=ax, shade = True, color= 'indianred')
# ax = sns.kdeplot(data=tb_ind['0'], label='Tb-Indian' , ax=ax, shade = True, color = 'skyblue')
...
tbx = healthy_tbx.append(tb_tbx)
montgo = healthy_montgo.append(tb_montgo)
shen = healthy_shen.append(tb_shen)
ind = healthy_ind.append(tb_ind)
...
feature_map_of_minist = model.predict(X_train[:300], verbose=1)
feature_map_of_minist_1 = model.predict(X_tb[:100], verbose=1)
